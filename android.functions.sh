...
for arg in "${@}";do
  ...
  if [[ "$arg" =~ '^--destination_dir$|^-d$|@The destination directory to store the pulled files - required' ]]; then local clip_seek_time=$1;continue;fi
...
source_files=$(adb shell find "${source_dir}" -type f)

if ! (echo -e "${source_files}" | while read source_file;do 
    echo "Pulling ${source_file} ..."
    if [[ ! $($PREFIX adb pull "${source_file}" "${2}") ]];then
      err=true
    fi
done);then 
  err=true;
